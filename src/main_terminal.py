"""
Electro-Thermal PINN: Terminal Mode (original main functionality).
"""
...
def generate_synthetic_data_with_varying_params(
    model, param_net, n_points=100, device=None, noise_level=0.02
):
...
def main_terminal():
    ...
    if enable_inverse:
        ...
        data_tuple = generate_synthetic_data_with_varying_params(
            reference_model, reference_param_net,
            n_points=n_data_points, device=device, noise_level=noise_level
        )
    ...
    for it in range(adam_iters):
        ...
        total_loss, loss_pde, loss_bc, loss_data, uncertainty = compute_loss(
            model, param_net, xi_f, xi_b0, xi_b1,
    ...
    if use_lbfgs:
        ...
        optimizer_lbfgs = torch.optim.LBFGS(
            list(model.parameters()) + list(param_net.parameters()),
            lr=1.0, max_iter=300, max_eval=300,
